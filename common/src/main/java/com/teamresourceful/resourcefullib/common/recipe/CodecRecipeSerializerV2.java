...
/**
 * Codec Recipe Serializer V2 uses YABN to send recipes over network as opposed to the previous version where it was a compressed json.
 */
public class CodecRecipeSerializerV2<R extends Recipe<?>> implements RecipeSerializer<R> {
    ...
    public CodecRecipeSerializerV2(RecipeType<R> recipeType, Function<ResourceLocation, Codec<R>> codec) {
        this(recipeType, codec, codec);
    }

    public CodecRecipeSerializerV2(RecipeType<R> recipeType, Function<ResourceLocation, Codec<R>> jsonCodec, Function<ResourceLocation, Codec<R>> networkCodec) {
